  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="max-w-6xl mx-auto px-6 py-24 md:py-32 space-y-32">
        ...
        <section className="flex flex-col lg:flex-row gap-16 items-center text-center lg:text-left">
          <div className="flex-1 space-y-10">
            ...
            <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
              <Button asChild size="lg" className="rounded-full px-10 h-14 text-base font-semibold shadow-xl shadow-black/5 hover:shadow-black/10 transition-all">
                <Link href="/work" className="no-underline">
                  Explore Work <ArrowRight className="ml-3 w-5 h-5" />
                </Link>
              </Button>
              <Button asChild variant="ghost" size="lg" className="rounded-full px-10 h-14 text-base font-semibold hover:bg-muted">
          ...
          <div className="w-full max-w-[500px] aspect-square relative group">
            <div className="absolute -inset-4 bg-muted/10 rounded-[3rem] blur-2xl group-hover:opacity-75 transition-opacity" />
            <div className="w-full h-full border border-border/50 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-black/5 bg-background relative z-10 transition-transform duration-500 group-hover:scale-[1.01]">
              <ThreeScene />
              <div className="absolute bottom-6 right-6 px-4 py-2 bg-background/80 backdrop-blur-md border border-border/50 rounded-2xl text-[10px] font-bold text-muted-foreground pointer-events-none">
                Interactive 3D
              </div>
            </div>
          </div>
        ...
        {/* Featured Section */}
        <section className="py-24 border-t border-border/50 space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <Link href="/work" className="group no-underline block p-12 border border-border/50 rounded-[3rem] hover:border-foreground transition-all bg-muted/5 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                <ArrowRight className="w-24 h-24 -rotate-45" />
              </div>
              <div className="relative z-10 space-y-6">
                <div className="flex gap-2">
                  <Badge className="bg-blue-50 text-blue-600 border-none rounded-md font-medium text-xs px-2 py-0.5 shadow-none">AI</Badge>
                  <Badge className="bg-purple-50 text-purple-600 border-none rounded-md font-medium text-xs px-2 py-0.5 shadow-none">Implementation</Badge>
                </div>
                <h3 className="text-3xl font-bold tracking-tight">Work History</h3>
            ...
            <Link href="/tools" className="group no-underline block p-12 border border-border/50 rounded-[3rem] hover:border-foreground transition-all bg-muted/5 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                <ArrowRight className="w-24 h-24 -rotate-45" />
              </div>
              <div className="relative z-10 space-y-6">
                <div className="flex gap-2">
                  <Badge className="bg-green-50 text-green-600 border-none rounded-md font-medium text-xs px-2 py-0.5 shadow-none">3D</Badge>
                  <Badge className="bg-orange-50 text-orange-600 border-none rounded-md font-medium text-xs px-2 py-0.5 shadow-none">Experiments</Badge>
                </div>
                <h3 className="text-3xl font-bold tracking-tight">Tools & Lab</h3>
                <p className="text-lg text-muted-foreground font-normal">Technical prototypes, mini-projects, and 3D experiments.</p>
                <div className="pt-4 flex items-center gap-2 font-bold text-sm">
                  Enter Laboratory <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>
            </Link>
          </div>
        ...
        {/* Footer Info */}
        <footer className="py-12 border-t border-border/50 flex flex-col md:flex-row justify-between items-center gap-8 text-sm font-medium text-muted-foreground">
